...
data RnnHypParams = RnnHypParams {
  ...
  , outputSize :: Int -- ^ The number of features in the output y
...
data SingleRnnParams = SingleRnnParams {
...
rnnCell :: SingleRnnParams 
  ...
  -> Tensor -- ^ ot' of shape <oDim>
rnnCell SingleRnnParams{..} ht xt = linearLayer rnnGate $ cat (Dim 0) [xt,ht]

rnnResizeOutput :: SingleRnnParams 
  -> Tensor -- ^ ht of shape <hDim>
  -> Tensor -- ^ ot' of shape <oDim>
rnnResizeOutput SingleRnnParams{..} ht = linearLayer outputGate ht
    ...
    then do -- the case of BiRNN
      ...
      let h0f = select 0 0 h0
          ...
          output1 = tmp
            ...
            .-> actf             -- | <seqLen, hDim> ??
          output2 = tmp
            .-> map (rnnResizeOutput singleRnnParams) -- | [<oDim>] 
            .-> tail -- | [<hDim>] of length seqLen (by removing h0)
            .-> stack (Dim 0)    -- | <seqLen, hDim>
            .-> actf             -- | <seqLen, hDim> ??
          hLast = output1                           -- | <seqLen, hDim>
            .-> sliceDim 0 (seqLen-1) seqLen 1     -- | <1, hDim>
            .-> (\o -> reshape (tail $ shape o) o) -- | <hDim>
      return (output2, hLast)
...
instance Randomizable RnnHypParams RnnParams where
  sample RnnHypParams{..} = do
    let xDim = inputSize
        ...
        xh2Dim = (d * hDim) + hDim
    RnnParams
      <$> (SingleRnnParams <$> sample (LinearHypParams dev hasBias xh1Dim hDim) <*> sample (LinearHypParams dev hasBias hDim oDim))
      <*> forM [2..numLayers] (\_ ->
        SingleRnnParams <$> sample (LinearHypParams dev hasBias xh2Dim hDim) <*> sample (LinearHypParams dev hasBias hDim oDim)
        )
